...
open class CInteropProcess: DefaultTask() {
    ...
    val outputFileName: String
        @Internal get() = with(CompilerOutputKind.LIBRARY) {
            ...
            return "$baseName-interop-$interopName$suffix"
        }
    ...
    val outputFileProvider = newOutputFile().apply {
        set {
            val targetSubDirectory = settings.target.disambiguationClassifier?.let { "$it/" }.orEmpty()
            project.buildDir.resolve("cinterop/$targetSubDirectory$interopName/$outputFileName")
        }
    }
    ...
    // We cannot use compileDependencyFiles here because this interop is one of theses files.
    val libraries: FileCollection
        @InputFiles get() = project.configurations.getByName(settings.compilation.compileDependencyConfigurationName)
